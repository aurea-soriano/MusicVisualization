...
public class WelchSplatting {
    ...
    public WelchSplatting(List<InstancePoint> listPoints, Integer width, Integer height, Integer fieldSize, float maxX, float maxY, float minX, float minY) {
        ...
        for (Integer i = 0; i < fieldSize; i++, x++) {
            ...
            for (Integer j = 0; j < fieldSize; j++, y++) {
                if (welchField.get(i).get(j) > 0) {
                    Float color = 1 - (splatValue * welchField.get(i).get(j));
    ...
    public List<List<Float>> createWelchSplatting() {
